<template>
  <div class="search-wrapper">
    ...
    <!-- Red validation alert — shown after submit attempt, disappears at 10 chars -->
    <transition name="alert-slide">
      ...
      >
        ...
        Please describe your case in at least 10 characters so we can find the right match for you.
        <span class="search-box__alert-count">{{ query.trim().length }}/10</span>
      </div>
    </transition>
...
const handleBlur = () => {
  ...
  if (query.value.trim().length > 0 && query.value.trim().length < 10) {
...
// Hide alert as soon as user reaches 10 characters
watch(() => query.value.trim().length, (len) => {
  if (len >= 10) showAlert.value = false
})
